...
Feature **100 % côté client** (spec `docs/superpowers/specs/2026-09-02-indice-perf-design.md`, correctif saturation §7.4 le 2026-09-08) : un score 0–1000 par pilote, par année et carrière, qui combine rang pondéré par les participants (`zScore` → `500 + 500·(z/√3)^2,5` pour z > 0, linéaire sous la médiane), constance par phase (±5 %, proxy « classé ⇒ finales » seulement sans phases KO publiées — Mondiaux), chrono transpondeur (z-score sur log temps, centré, strict — poids `PERF_CHRONO_W` = 0,3), classements de séries (même z, année = dernier événement), sous coefs de niveau légers (Régional 0,93 / National 1,0 / UCI 1,05, clamp [5,1000]). DNF/DNS/DSQ final = pénalité graduée par la phase atteinte (`PERF_DNF_SCORES` : finale ≈ 700 · demi ≈ 550 · quart ≈ 400 · manche/inconnu ≈ 250, × coef).
